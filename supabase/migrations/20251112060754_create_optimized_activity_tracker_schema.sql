/*
  # Optimized Activity Tracker Database Schema
  ...
  2. Indexes
    - Composite index on (device_id, timestamp) for efficient per-device queries with date ordering
    - Index on timestamp for global date-based sorting (if needed)
    - Event type and package indexes for filtering

  3. Security
    - Enable RLS on both tables
    - Add policies for device-specific data access
    - Secure function with immutable search_path to prevent injection attacks

  4. Performance
    - Optimized indexes based on actual query patterns
    - Automatic app_metadata updates via trigger
*/
...
-- Most important: device_id + timestamp for per-device pagination and sorting
CREATE INDEX IF NOT EXISTS idx_activity_events_device_timestamp 
  ON activity_events(device_id, timestamp DESC);

-- For filtering by event type within device context
CREATE INDEX IF NOT EXISTS idx_activity_events_device_type 
  ON activity_events(device_id, event_type, timestamp DESC);

-- For app-specific queries
CREATE INDEX IF NOT EXISTS idx_activity_events_device_package 
  ON activity_events(device_id, app_package_name, timestamp DESC);

-- For app_metadata lookups
CREATE INDEX IF NOT EXISTS idx_app_metadata_package 
  ON app_metadata(package_name);
...
-- Create a SECURE function to update app_metadata when new events are inserted
...
AS $$
BEGIN
  INSERT INTO public.app_metadata (package_name, app_name, last_seen, event_count)
  VALUES (NEW.app_package_name, NEW.app_name, NEW.timestamp, 1)
  ON CONFLICT (package_name) 
  DO UPDATE SET
    app_name = EXCLUDED.app_name,
    last_seen = EXCLUDED.last_seen,
    event_count = public.app_metadata.event_count + 1;
  RETURN NEW;
END;
$$;
